...
/*   Updated: 2025/08/23 18:47:08 by asaadeh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void free_all_and_exit(t_parsing *parsing, t_vars *vars, t_directions *dirs);
int get_last_line(t_vars *vars, t_parsing *parsing);
void check_elements(t_parsing *parsing, t_vars *vars, t_directions *dirs);
...
int get_first_non_whitespace(char *line)
{
	int j = 0;
	while (line[j] && line[j] != '\n')
	{
		if (is_non_whitespace(line[j]))
			return j;
		j++;
	}
	return -1;
...
int get_last_non_whitespace(char *line)
{
	int j = 0;
	int last = -1;
	while (line[j] && line[j] != '\n')
...
void report_empty_line_error(t_parsing *parsing, t_vars *vars, t_directions *dirs)
{
	printf("Error\nEmpty line inside map\n");
	free_all_and_exit(parsing, vars, dirs);
}

void report_tab_error_msg(int i, int j)
{
	printf("Error\nTab found at line %d, column %d\n", i, j);
}

void report_tab_error(t_parsing *parsing, t_vars *vars, t_directions *dirs, int i, int j)
{
	report_tab_error_msg(i, j);
	free_all_and_exit(parsing, vars, dirs);
}

void check_tabs_in_bounds(char *line, int i, int start, int end,
						  t_parsing *parsing, t_vars *vars, t_directions *dirs)
{
	int j = start;
	while (j <= end)
	{
		if (line[j] == '\t')
			report_tab_error(parsing, vars, dirs, i, j);
		j++;
	}
}

void validate_map_line(int i, t_vars *vars,
					   t_parsing *parsing, t_directions *dirs)
{
	char *line = vars->map[i];
	int first = get_first_non_whitespace(line);
	int last = get_last_non_whitespace(line);

	if (first == -1)
		report_empty_line_error(parsing, vars, dirs);

	check_tabs_in_bounds(line, i, first, last, parsing, vars, dirs);
}
