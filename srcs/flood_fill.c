...
/*   Updated: 2025/08/19 14:22:11 by asaadeh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int flood_from_player(t_vars *vars, int y, int x)
{
    ...
    int row_len = ft_strlen(vars->map[y]);
    if (x < 0 || x >= row_len)
        return 1;
    ...
    if (flood_from_player(vars, y + 1, x)) return 1;
    if (flood_from_player(vars, y - 1, x)) return 1;
    if (flood_from_player(vars, y, x + 1)) return 1;
    if (flood_from_player(vars, y, x - 1)) return 1;
...
int flood_from_remaining_zero(t_vars *vars, int y, int x)
{
    ...
    if (flood_from_remaining_zero(vars, y + 1, x)) return 1;
    if (flood_from_remaining_zero(vars, y - 1, x)) return 1;
    if (flood_from_remaining_zero(vars, y, x + 1)) return 1;
    if (flood_from_remaining_zero(vars, y, x - 1)) return 1;
...
{
    int start_x = (int)vars->xp_pos;
    int start_y = (int)vars->yp_pos;
    ...
    while (y < vars->map_height)
    {
        ...
        while ((size_t)x < ft_strlen(vars->map[y]))
        {
            if (vars->map[y][x] == '0')
            {
                ...
                {
                    printf("Error: Map contains open area not reachable\n");
                    return 1;
                }
            }
            x++;
